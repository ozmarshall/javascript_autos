...
function obtenerVehiculos() {
  fetch("https://61ef3d66d593d20017dbb3ad.mockapi.io/Vehiculos")
    .then(function (respuesta) {
      return respuesta.json();
    })
    .then(function (computers) {
      document.getElementById("app").innerHTML = "";
      computers.forEach(function (carro) {
        // se recorre coleccion
        agregarNodoEnHtml(crearNodo(carro));
      });
    });

  function crearNodo(carro) {
    console.log("carro", carro);
    const nodo = document.createElement("article");
    nodo.innerHTML = `
      <article class="card mb-4">
          <div class="card-body">
              <h5  class="card-title">Marca: ${carro.marca}</h5>
              <p class="card-text">Memoria: ${carro.modelo}</p>
              <p class="card-text">Color: ${carro.color}</p>
              <img style="width:100%" class="img-full" src=${carro.photo}>
    ...
    nodo.querySelector(".js_delete").onclick = function () {
      if (confirm("Estas seguro que quieres eliminar el elemento")) {
        fetch(
          `https://61ef3d66d593d20017dbb3ad.mockapi.io/Vehiculos/${carro.id}`,
          {
            method: "DELETE",
          }
        )
          ...
          .then((respuesta) => {
            ...
            nodo.remove();
            alert(`Se elimino correctamente el id ${carro.id}`);
          })
          .catch((error) =>
            alert(
              `No se pudo eliminar el id ${carro.id}, por favor intentelo despues`
            )
          );
      }
    };
    ...
    nodo.querySelector(".js_edit").onclick = function () {
      ...
      const inputModelo = document.querySelector(".js_form_edit .js_modelo");
      ...
      inputColor.value = carro.color;
      inputPhoto.value = carro.photo;
      inputId.value = carro.id;
      // refFormulario.marca.value = computadora.marca
      // refFormulario.modelo.value = computadora.modelo
...
function obtenerValores(form) {
  return {
    ...
    modelo: form.querySelector(`.js_modelo`).value,
...
function obtenerReferenciasFormulario(form) {
  return {
    ...
    modelo: form.querySelector(`.js_modelo`),
...
function registrarEventoFormulario() {
  ...
  //     modelo: document.querySelector(".js_modelo").value,
...
function registrarEventoFormularioEdit() {
  ...
  form.onsubmit = function (event) {
    ...
    fetch(`https://61ef3d66d593d20017dbb3ad.mockapi.io/Vehiculos/${idValue}`, {
      method: "PUT",
      body: JSON.stringify(obtenerValores(form)),
      headers: {
        "Content-Type": "application/json",
